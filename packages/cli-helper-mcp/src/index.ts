...
function capOutput(s: string, max = 10000): string {
...
function runCmd(cmd: string, args_: string[], timeoutMs = 120000): { stdout: string; success: boolean; stderr?: string } {
  try {
...
function toolShowDialog(args: {
  title: string;
  message: string;
  mode?: 'confirm' | 'ok' | 'input' | 'select' | 'file_picker';
  choices?: string[];
  fileFilter?: string;
  multiSelect?: boolean;
  continueAdding?: boolean;  // file_picker only: loop until user cancels "Add more?"
}): object {
...
// ─── show_asset_picker ───────────────────────────────────────────────────────
//
// Opens a browser tab showing image assets side-by-side for comparison,
// then shows a selection dialog. The browser tab is non-blocking — the user
// can see all assets while making their choice in the dialog.
//
// Returns: { selected: string (label), index: number } or { cancelled: true }

import type { AssetPickerItem, PickerConfig } from './picker/types.js';

// ─── Browser-based asset picker (session store) ──────────────────────────────

interface PickerSession {
  items: AssetPickerItem[];       // mutable — upload appends here
  multiSelect: boolean;
  uploadDir?: string;
  resolve: (v: {
...
const _pickerSessions = new Map<string, PickerSession>();
...
interface UploadSession {
  saveDir: string;
  savedFiles: Array<{ name: string; path: string; size: number }>;
  resolve: (v: { files: Array<{ name: string; path: string; size: number }>; cancelled: boolean }) => void;
}

const _uploadSessions = new Map<string, UploadSession>();

function buildUploadHtml(sessionId: string, title: string, message: string): string {
  function esc(s: string) {
    return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
  }
  return `<!DOCTYPE html>
...
async function toolUploadFiles(args: {
  ...
  const result = await new Promise<{ files: Array<{ name: string; path: string; size: number }>; cancelled: boolean }>((resolve) => {
...
function toolCheckProcess(args: { processName: string }): object {
...
const server = new Server(
  { name: 'cli-helper-mcp', version: '2.0.0' },
  { capabilities: { tools: {} } }
);
  ...
  try {
    switch (name) {
      ...
      case 'show_notification':
        ...
        break;
      case 'run_command':
        result = toolRunCommand(args as { command: string; cwd?: string; timeoutMs?: number });
        break;
      case 'check_process':
        result = toolCheckProcess(args as { processName: string });
        break;
      case 'open_path':
        result = toolOpenPath(args as { path: string });
        break;
      case 'upload_files':
        result = await toolUploadFiles(args as { title: string; message: string; saveDir: string });
        break;
      default:
...
function apiJson(res: http.ServerResponse, data: unknown) {
  ...
  const srv = http.createServer((req, res) => {
    ...
    if (pickApiMatch && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        try {
          const { indices, uploaded, cancelled } = JSON.parse(body) as {
            indices: number[];
    ...
    if (pickerUploadMatch && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        if (!session || !session.uploadDir) {
          res.writeHead(404); res.end('Session not found or upload not enabled'); return;
        }
        try {
          const { name, data, size } = JSON.parse(body) as { name: string; data: string; mime: string; size: number };
          ...
          const newItem: AssetPickerItem = { label: safeName, imagePath: dest, metadata: { _src: 'uploaded', size: `${Math.round(size / 1024)}KB` } };
    ...
    if (uploadApiMatch && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        try {
          const { files, cancelled } = JSON.parse(body) as {
